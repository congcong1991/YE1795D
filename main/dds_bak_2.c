...
uint16_t  sqrt_32_fast(uint32_t  x)
{
		uint32_t u;
		if(x==0)return(0);
		if( x > 0x0000ffff){
...
void next_dds(int32_t cur_v){
	...
	{	
		...
		dds_p.udc=(uint32_t)config.hv_set*100;   // 
		dds_p.u1=(((int64_t)sin0*va)>>16)	+	dds_p.udc;   // mV
		du=dds_p.u1-dds_p.u0;
//		tempt=(int64_t)dds_p.u0*BOOSTL*2*((int64_t)dds_p.u0*PWMT/RS+(int64_t)dds_p.cc*du)/LOWU/LOWU;
  // mV*uH(mV*nS/K欧+pF*mV)/mV/mV
	//	if(tempt>0)
		if(1)
			{  // 电感充电
	//		dds_p.tns1=sqrt_32_fast(tempt)+MIN_PW0;   //  sqrt(10-18 J* uH)/ mV = nS
			dds_p.tns1=(dds_p.u1>>5)+MIN_PW0;   //  sqrt(10-18 J* uH)/ mV = nS

			TIM1->CCR1=(uint32_t)dds_p.tns1*197>>12;
			TIM1->CCR2=0;
		}else if(tempt<0){  // 电阻放电
			dds_n.tns2=-((int64_t)dds_p.u0*PWMT/1750/RS+(int64_t)dds_p.cc*du/1750); 
			// mV*nS/uA/K欧=nS,, pF*mV/uA=nS
			TIM1->CCR1=0;
...
//	while(0)
	{
		...
		dds_n.udc=(uint32_t)config.hv_set*100;   // 
		dds_n.u1=(((int64_t)sin0*va)>>16)	+	dds_n.udc;   // mV
		du=dds_n.u1-dds_n.u0;
//		tempt=(int64_t)dds_p.u0*BOOSTL*2*((int64_t)dds_p.u0*PWMT/RS+(int64_t)dds_p.cc*du)/LOWU/LOWU;
  // mV*uH(mV*nS/K欧+pF*mV)/mV/mV
	//	if(tempt>0)
		if(1)
			{  // 电感充电
	//		dds_p.tns1=sqrt_32_fast(tempt)+MIN_PW0;   //  sqrt(10-18 J* uH)/ mV = nS
			dds_n.tns1=(dds_n.u1>>5)+MIN_PW0;   //  sqrt(10-18 J* uH)/ mV = nS

			TIM1->CCR3=(uint32_t)dds_n.tns1*197>>12;
			TIM1->CCR4=0;
		}
	}
	dds_p.phase+=dphase;
