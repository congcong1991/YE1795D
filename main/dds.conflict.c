...
void next_dds(int32_t cur_v){
	...
	if(max_pw>MAX_PW0)max_pw=MAX_PW0;
		...
		temprate=(uint32_t)config.hv_set*124878L/parameter.currDC;  //  放大了256倍
...
//				dds_p.tns1=dds_p.tns1*temprate>>8;
				dds_p.tns1+=MIN_PW0;   //  nS
				if(dds_p.tns1>max_pw)dds_p.tns1=max_pw;
				if((parameter.up>165000)){	TIM1->CCR3=0;}
...
//		tab_sqrt_trig(dds_n.phase,&cos0,&sqrt_div0,&sqrt_mul0);
		...
		if(ndu0>0)
			{  // 充电
				...
				else {		TIM1->CCR4=((uint32_t)dds_n.tns1*197>>12); }
				TIM1->CCR2=0;
		}else if(ndu0<0){  // 放电
